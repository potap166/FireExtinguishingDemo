 ...
 uint32_t GetLSEFrequency(void)
{
  ...
  HAL_TIMEx_RemapConfig(&TimInputCaptureHandle, TIM_TIM5_LSE);
  ...
  HAL_TIM_IC_DeInit(&TimInputCaptureHandle);
   ...
   if(HAL_TIM_IC_Stop_IT(&TimInputCaptureHandle, TIM_CHANNEL_4) != HAL_OK)
  {
  }
  ...
  { 
    ...
    uint32_t  gg =(((2 * pclk1) / uwPeriodValue)*8);
    return gg;
  }
}
/**
