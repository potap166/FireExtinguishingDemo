...
static osMutexId_t     osMutexGUI  @"SDRAM_STATIC_VAR"        ; 
static osSemaphoreId_t SemGui      @"SDRAM_STATIC_VAR"        ; 
...
1 ms.
*/
const osMutexAttr_t   GUI_Mutex_attributes = {
...
{ 
  /* Create Mutex lock */
 // osMutexDef(MUTEX);
  osMutexGUI = osMutexNew ( &GUI_Mutex_attributes );
  
  
  
  SemGui = osSemaphoreNew(5, 0, NULL);
...
{ 
 // osMutexRelease(osMutex);
   osMutexRelease (osMutexGUI);
}

void GUI_X_Lock(void)
{
  osMutexAcquire ( osMutexGUI , osWaitForever ) ; 
...
void GUI_X_WaitEvent (void) 
{
 // osSemaphoreWait(osSemaphore , osWaitForever) ;
  osSemaphoreAcquire(SemGui, osWaitForever);
}


void GUI_X_SignalEvent (void) 
{
    osSemaphoreRelease(SemGui);
