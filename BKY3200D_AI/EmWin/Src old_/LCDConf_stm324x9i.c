...
#define LCD_LAYER0_FRAME_BUFFER  (uint32_t)0xD0000000U 
//#define LCD_LAYER0_FRAME_BUFFER  (uint32_t)0xD0000000U + (uint32_t)(1024U*1024U)
#define LCD_LAYER1_FRAME_BUFFER  LCD_LAYER0_FRAME_BUFFER + ((uint32_t))(480*800*2)
...
static void LCD_DrawBitmap8bpp(int32_t LayerIndex, int32_t x, int32_t y, U8  * p, int32_t xSize, int32_t ySize, int32_t BytesPerLine);
static void LCD_DrawBitmap16bpp(int32_t LayerIndex, int32_t x, int32_t y, U16  * p, int32_t xSize, int32_t ySize, int32_t BytesPerLine);
...
{  
 ...
 // __HAL_RCC_GPIOK_CLK_ENABLE();  
  ...
  /* Configure peripheral GPIO */
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_9|GPIO_PIN_10|GPIO_PIN_2|GPIO_PIN_4|GPIO_PIN_5;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    GPIO_InitStruct.Alternate = GPIO_AF14_LTDC;
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_8|GPIO_PIN_10 |GPIO_PIN_9
...
#if (GUI_NUM_LAYERS>1)
  layer_prop[1].address =  LCD_LAYER0_FRAME_BUFFER + (480*800*2);    
#endif
...
void LCD_CtrlLinesConfig(void)
{
  ...
  GPIO_InitTypeDef GPIO_InitStruct;
  GPIO_InitStruct.Pin = GPIO_PIN_3 ;                                             //направление передачи
...
static void LCD_LL_Init(void) 
{
  ...
  uint32_t HSYNC=0U,VSYNC=0U,VFP=0U,HFP=0U,VBP=0U,HBP=0U;
  uint32_t ACT_W=0U,ACT_H=0U,TOTL_H=0U,TOTL_W=0U;
  
  
 // LCD_CtrlLinesConfig();
  //HAL_GPIO_WritePin(GPIOF,GPIO_PIN_10 ,GPIO_PIN_RESET) ; // LCD_ChipSelect(DISABLE);
 // HAL_GPIO_WritePin(GPIOF,GPIO_PIN_10 ,GPIO_PIN_SET) ; // LCD_ChipSelect(ENABLE);
  ...
  //PeriphClkInitStruct.PLLSAI.PLLSAIN =240;       //66 70
  ...
  */
  ...
  /*
   ...
   hltdc.Init.AccumulatedActiveW = ACT_W-1;
  hltdc.Init.AccumulatedActiveH = ACT_H-1;
   hltdc.Init.TotalWidth = TOTL_W-1;
   hltdc.Init.TotalHeigh = TOTL_H-1; */
